 ...
 * This program and all supporting files are distributed under the Limited GNU
 * Public License (LGPL, http://www.gnu.org).
 * 
 * This is a very simple object that allows the TheadPool to determine when it
 * is done. This object implements a simple lock that the ThreadPool class can
 * wait on to determine completion. Done is defined as the ThreadPool having no
...
public class Done {
    ...
    synchronized public void workerBegin() {
        //System.out.println("active Threads="+ _activeThreads);
        _activeThreads++;
        _started = true;
        notify();
    }
