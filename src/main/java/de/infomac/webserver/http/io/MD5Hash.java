...
// $Id: MD5Hash.java,v 1.1 2006/10/23 22:03:36 matteo Exp $
//
...
//		 fill the hash accumulator with a seed value
	 ...
	 *  
	 */
	private static MD5Hash md = new MD5Hash();

	public static String hashOf(byte[] thing, int length) {
		byte[] hash = new byte[16];

		md.clear();

		md.update(thing, length);
		md.md5final(hash);

		return MD5Hash.dumpBytes(hash);
	}
